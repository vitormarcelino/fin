 ...
 * splits them into overdue / upcoming / other based on `dueDate`.
 */
...
export type PendingBuckets = {
  ...
  /** dueDate is set and between `today` and `upcomingUntil`, inclusive. */
...
export function bucketPendingEntries(
  ...
  const buckets: PendingBuckets = { overdue: [], upcoming: [], other: [] };
