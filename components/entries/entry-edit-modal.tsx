...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="flex h-12 w-full items-center justify-center rounded-xl bg-slate-900 text-white font-medium text-base active:opacity-80 disabled:opacity-60"
    >
  ...
  return (
    ...
    >
      <div
        ...
        className="flex w-full max-w-sm flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-6 text-slate-900 shadow-lg"
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Fechar"
            className="shrink-0 rounded-full p-1 text-slate-500 active:opacity-70"
          >
            ✕
          </button>
        </div>
            ...
            <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Status do lançamento">
              {STATUS_OPTIONS.map((opt) => (
                <label key={opt.value} className="relative">
                  ...
                  />
                  <span className="flex h-12 w-full cursor-pointer items-center justify-center rounded-xl border border-slate-200 text-base font-medium text-slate-700 active:opacity-80 peer-checked:border-slate-900 peer-checked:bg-slate-900 peer-checked:text-white">
                    {opt.label}
                  </span>
                </label>
              ))}
        ...
        <Link
          ...
          className="text-center text-sm font-medium text-slate-600 active:opacity-70"
        >
